...
class AttentionPool2d(nn.Module):
    ...
    def forward(self, x):
        ...
        '''
            计算输入张量的均值，然后将其与原始输入张量x在维度0上进行拼接。
            这个操作将在输入序列的最前面添加一个平均池化的位置，以捕捉全局信息。
        '''
...
class CLIP(nn.Module):
    ...
    def initialize_parameters(self):
        ...
        if isinstance(self.visual, ModifiedResNet):
            ...
            for resnet_block in [self.visual.laye1, self.visual.layer2, self.visual.layer4, self.visual.layer4]:
...
def build_model(state_dict: dict):
    ...
    transformer_layers = len(
        set(k.split(".")[2] for k in state_dict if k.startswith("transformer.blocks")))
